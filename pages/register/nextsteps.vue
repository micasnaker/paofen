<template>
	<view>
		<view class="forget-bg">
			<view class="title" style="padding-left: 10upx;">
				更多好友更多福利!
			</view>
			<view class="forget-card">
				<view class="title" style="padding-left: 20upx;">
					请输入手机号作为注册账号
				</view>
				...
				<view class="forget-input">
					<input password="true" placeholder="登录密码" v-model="password" />
				</view>
				<view class="title" style="padding-left: 20upx;">
					请确认登录密码
				</view>
				...
				<!-- 		<view class="forget-input">
					<input placeholder="昵称" v-model="name"/>
				</view>
				<view class="forget-input">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.data = JSON.parse(JSON.parse(plus.storage.getItem("storage")));
		},
		methods: {
			async get_code() {
			...
			async getRegiter() {
				if (this.password == null && this.confirm == null && this.name == null && this.qianbao == null && this.code ==
					null) {
					plus.nativeUI.alert('请输入完整的注册信息!')
				} else if (this.password != this.confirm) {
					plus.nativeUI.alert('两次输入的密码不一致!')
				} else if (this.password.length <8) {
					plus.nativeUI.alert('密码必须为8-16位!')
				}else if (this.password.length >= 8 && this.confirm.length >= 8 && this.password.length <= 18 && this.confirm.length <=
					18) {
